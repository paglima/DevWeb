 ...
 * @author pauloarthur
 */
@WebServlet(name = "DepositoServlet", urlPatterns = {"/deposita"})
public class DepositoServlet extends GenericServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            valorDeposito = Double.valueOf( request.getParameter("valorDeposito") );
        }catch(Exception e){
            request.setAttribute("mensagemErro", "Preencha o campo corretamente");
            request.getSession().setAttribute("usuario",usuario);
            request.getServletContext().getRequestDispatcher("/deposito.jsp").forward(request, response);
            return;
        }
         ...
         try {
             ...
             Extrato extrato = new Extrato("Depositou " + valorDeposito+ " R$", conta.getSaldo(), getTimestamp(), conta);
             extradoDAO.save(extrato);
             
             request.getSession().setAttribute("usuario",usuarioBanco);
             request.setAttribute("mensagemSucesso", "Depósito feito com sucesso!");
